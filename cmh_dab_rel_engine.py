...
def evaluate(model, criterion, postprocessors, data_loader, base_ds, device, args):
    ...
    for samples, targets in metric_logger.log_every(data_loader, 100, header):
        ...
        #SGG eval
        if args.dataset == 'vg':
            evaluate_rel_batch_sig(outputs, targets, evaluator, evaluator_list)
        else:
...
def evaluate_rel_batch_sig(outputs, targets, evaluator, evaluator_list):
    ...
    # rel_prob = rel_prob.view(outputs['rel_logits'].shape[0], -1) #[bs,30600]

    # sub_topk_values, sub_topk_indexes = torch.topk(sub_prob.view(outputs['sub_logits'].shape[0], -1), num_select, dim=1)
    # sub_scores = sub_topk_values

    # ojb_topk_values, ojb_topk_indexes = torch.topk(ojb_prob.view(outputs['ojb_logits'].shape[0], -1), num_select, dim=1)
    # ojb_scores = ojb_topk_values
    ...
    for batch, target in enumerate(targets):
        ...
        # #sub_bboxes_scaled   obj_bboxes_scaled 都是[600,4]的np array
        # sub_bboxes_scaled = rescale_bboxes(outputs['sub_boxes'][batch].cpu(), torch.flip(target['orig_size'],dims=[0]).cpu()).clone().numpy()
        # obj_bboxes_scaled = rescale_bboxes(outputs['obj_boxes'][batch].cpu(), torch.flip(target['orig_size'],dims=[0]).cpu()).clone().numpy()
