...
class Transformer(nn.Module):
    ...
    def forward(self, src, mask, entity_embed, triplet_embed, pos_embed, so_embed):
        ...
        bs, c, h, w = src.shape # bs 256 25 40
        
        src = src.flatten(2).permute(2, 0, 1) # torch.Size([1000, 4, 256])
        pos_embed = pos_embed.flatten(2).permute(2, 0, 1) #torch.Size([1000, 4, 256])
        ...
        # hs torch.Size([6, 100, bs, 256])    hs_t torch.Size([6, 200, bs, 512])  sub_maps torch.Size([6, bs, 200, 1000])

        #so_masks torch.Size([6, bs, 200, 2, 25, 40])
...
class TransformerDecoder(nn.Module):
    ...
    def forward(self, entity, triplet, memory,
        ...
        for layer in self.layers:
            ...
            #output_entity [100,bs,256]   output_triplet[200,bs,512]    sub_maps, obj_maps [bs,200,1000] 1000是backbone feature的长度
...
class TransformerDecoderLayer(nn.Module):
    ...
    def forward(self, tgt_entity, tgt_triplet, entity_pos, triplet_pos, so_pos,
        ...
        tgt2_entity = self.self_attn_entity(q_entity, k_entity, value=tgt_entity, attn_mask=tgt_mask,
                                            key_padding_mask=tgt_key_padding_mask)[0] #torch.Size([100, bs, 256]) 
        tgt_entity = tgt_entity + self.dropout2_entity(tgt2_entity)#torch.Size([100, bs, 256]) 
        ...
        tgt2_entity = self.cross_attn_entity(query=self.with_pos_embed(tgt_entity, entity_pos),
                                             ...
                                             key_padding_mask=memory_key_padding_mask)[0] #torch.Size([100, bs, 256]) 
        tgt_entity = tgt_entity + self.dropout1_entity(tgt2_entity) # torch.Size([100, bs, 256])
        tgt_entity = self.norm1_entity(tgt_entity) #torch.Size([100, bs, 256]) 
        ...
        tgt_sub, tgt_obj = torch.split(tgt_triplet, h_dim, dim=-1) #torch.Size([200, bs, 256]) torch.Size([200, bs, 256])
        ...
        tgt2_so = self.self_attn_so(q_so, k_so, tgt_so)[0] # torch.Size([400, bs 256])
        tgt_so = tgt_so + self.dropout2_so(tgt2_so) # torch.Size([400, bs 256])
        tgt_so = self.norm2_so(tgt_so) # torch.Size([400, bs 256])
        tgt_sub, tgt_obj = torch.split(tgt_so, t_num, dim=0)   #torch.Size([200, bs, 256]) #torch.Size([200, bs, 256])

        # subject branch - decoupled visual attention
        # tgt2_sub torch.Size([200, bs, 256])   sub_maps[bs, 200, 1000]
        tgt2_sub, sub_maps = self.cross_attn_sub(query=self.with_pos_embed(tgt_sub, triplet_pos),
                                                 key=self.with_pos_embed(memory, pos),
                                                 value=memory, attn_mask=memory_mask,
                                                 key_padding_mask=memory_key_padding_mask)
        tgt_sub = tgt_sub + self.dropout1_sub(tgt2_sub) #torch.Size([200, bs, 256])
        tgt_sub = self.norm1_sub(tgt_sub) # torch.Size([200, bs, 256])        

        # subject branch - decoupled entity attention
        tgt2_sub = self.cross_sub_entity(query=self.with_pos_embed(tgt_sub, triplet_pos),
                                         key=tgt_entity, value=tgt_entity)[0] #torch.Size([200, bs, 256])
        tgt_sub = tgt_sub + self.dropout2_sub(tgt2_sub) #torch.Size([200, bs, 256])
        tgt_sub = self.norm2_sub(tgt_sub) #torch.Size([200, bs, 256])
        tgt_sub = self.forward_ffn_sub(tgt_sub)  # torch.Size([200, bs, 256])

        # object branch - decoupled visual attention
        # tgt2_obj torch.Size([200, bs, 256])   obj_maps[bs, 200, 1000]
        ...
        tgt_obj = self.norm1_obj(tgt_obj) # torch.Size([200, bs, 256])

        # object branch - decoupled entity attention
        tgt2_obj = self.cross_obj_entity(query=self.with_pos_embed(tgt_obj, triplet_pos),
                                         key=tgt_entity, value=tgt_entity)[0] # torch.Size([200, bs, 256])
        tgt_obj = tgt_obj + self.dropout2_obj(tgt2_obj) # torch.Size([200, bs, 256])
        tgt_obj = self.norm2_obj(tgt_obj) # torch.Size([200, bs, 256])
