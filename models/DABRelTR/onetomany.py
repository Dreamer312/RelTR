...
# from cmh_util.box_ops import box_cxcywh_to_xyxy, generalized_box_iou, box_iou, box_xyxy_to_cxcywh
...
def sample_topk(pr_inds, gt_inds, cost_matrix, k):
    ...
    gt_inds2, counts = gt_inds.unique(return_counts=True) # 返回独一无二的类别  和每个类别的出现次数    当前是 size[7]  size[7]
    scores, pr_inds2 = cost_matrix[gt_inds2].topk(k, dim=1) # pr_inds2    size[7,6]   7个类别，每个类别选最匹配的6个query
    gt_inds2 = gt_inds2[:, None].repeat(1, k) # size[7,6]  
...
class Stage2Assigner(nn.Module):
    ...
    def _sample_proposals(
        ...
        if has_gt:
            gt_classes = gt_classes[matched_idxs] # 900
    ...
    def get_cost_matrix(self, pred_logits, pred_boxes, gt_classes, gt_boxes):
        ...
        num_queries = len(pred_logits) # 900
        out_prob = pred_logits.sigmoid() # 900 151
        out_bbox = pred_boxes # 900 4
        cost_box = box_iou(box_cxcywh_to_xyxy(out_bbox), box_cxcywh_to_xyxy(gt_boxes))[0] # torch size 900 * 18
        cost_class = out_prob[:, gt_classes] # 900 * 18
        C = 0.7 * cost_box + 0.3 * cost_class
        C = C.view(num_queries, -1) # 900 * 18
        return C.T
    ...
    def forward(self, outputs, targets, return_cost_matrix=False):
        ...
        cost_matrices = []
        for b in range(bs):
            pred_logits = outputs['pred_logits_cdecoder'][b].detach()  # 200 151
            pred_boxes = outputs['pred_boxes'][b]  # 200 151
            gt_boxes = targets[b]['boxes'] # 2 4
            gt_classes = targets[b]['labels'] # 2
            cost_matrix = self.get_cost_matrix(pred_logits, pred_boxes, gt_classes, gt_boxes) # [2 200]     2指当前图片有2个box

            # matched_idxs 900        matched_labels 900
            ...
            # sampled_idxs 300  sampled_gt_classes 300
            sampled_idxs, sampled_gt_classes = self._sample_proposals(
                matched_idxs, matched_labels, targets[b]['labels']
            )
            pos_pr_inds = sampled_idxs[sampled_gt_classes != self.bg_label] # 17
            pos_gt_inds = matched_idxs[pos_pr_inds] # 17
            pos_pr_inds, pos_gt_inds = self.postprocess_indices(pos_pr_inds, pos_gt_inds, cost_matrix) # 17 17
